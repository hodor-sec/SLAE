/*
Filename: aes-128-cbc_enc-dec.c
Author: hodorsec
Description: Encrypts a file as input with a given password and outputs in C hex
Loosely based on URL with some fixes and checks: http://www.firmcodes.com/how-do-aes-128-bit-cbc-mode-encryption-c-programming-code-openssl/
Compile with: gcc -fno-stack-protector -z execstack -fno-pie -o aes-128-cbc_enc-dec aes-128-cbc_enc-dec.c  -lcrypto -I /usr/include/openssl -L /usr/lib/
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv)
{
	...
	unsigned char ivDec[AES_BLOCK_SIZE] = { 0 };
	...
	// void AES_cbc_encrypt(const unsigned char *in, unsigned char *out, size_t length, const AES_KEY *key, unsigned char *ivec, const int enc);	
	AES_cbc_encrypt(shell_enc, shell_dec, lenDec, &decKey, ivDec, AES_DECRYPT); // AES_DECRYPT == 0 from <openssl/aes.h>
	
	// Output
	print_data("\nORIGINAL", shell_in, lenShell);
	print_data("\nENCRYPTED", shell_enc, lenEnc);
	print_data("\nDECRYPTED", shell_dec, lenShell);
	printf("\n");
	return 0;
}
